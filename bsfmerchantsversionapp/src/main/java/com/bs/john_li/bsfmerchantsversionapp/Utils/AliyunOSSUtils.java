...
public class AliyunOSSUtils {
    ...
    public static OSSClient initOSS(Context context) {
        ...
        if (BSFMerchantConfigtor.OSS_TOKEN .equals("")) {
            credentialProvider = new STSGetter(context);
        }else {
            credentialProvider = new STSGetter(context, BSFMerchantConfigtor.BASE_URL + BSFMerchantConfigtor.OSS_TOKEN);
        }
        //初始化OSSClient
    ...
    private void putImg(final Handler mHandler, OSSClient oss, File file) {
        Bitmap bitmap = BSFCommonUtils.compressImageFromFile(file.getPath(), 1024f);// 按尺寸压缩图片
        File filePut = BSFCommonUtils.compressImage(bitmap, file.getPath());  //按质量压缩图片

        String fileName = filePut.getName();
        String filePath = filePut.getPath();
        // 构造上传请求
        PutObjectRequest put = new PutObjectRequest(BSFMerchantConfigtor.BucketName, fileName, filePath);
    ...
    public static void downloadImg(String object, OSSClient oss, final ImageView iv, final Context context, final int fialImgRes) {
        ...
        GetObjectRequest get = new GetObjectRequest(BSFMerchantConfigtor.BucketName, object);
        ...
        OSSAsyncTask task = oss.asyncGetObject(get, new OSSCompletedCallback<GetObjectRequest, GetObjectResult>() {
            ...
            public void onSuccess(GetObjectRequest request, GetObjectResult result) {
                ...
                opt.inSampleSize= BSFCommonUtils.calculateInSampleSize(opt,carPhotoIv.getWidth(), carPhotoIv.getHeight());
                ...
                *//*try {
                    ...
                    //需要根据对应的View大小来自适应缩放
                    Bitmap bm = BSFCommonUtils.autoResizeFromStream(inputStream, carPhotoIv);
                    inputStream.close();
                ...
                *//*try {
                ...
                try {
                    ...
                    date = BSFCommonUtils.readStream(inputStream);
                    //获取bitmap
